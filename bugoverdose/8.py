# Runtime: 40 ms, faster than 17.40% of Python3 online submissions for String to Integer (atoi).
# Memory Usage: 14.3 MB, less than 54.36% of Python3 online submissions for String to Integer (atoi).
class Solution:
    def myAtoi(_, s: str) -> int:
        final_str = ""
        s = s.strip()
        plus_minus = 1
        ...
        try:
            for i in range(len(s)):
                final_str += str(int(s[i]))
        finally: 
            if final_str == "":
                return 0
            final_num = plus_minus * int(final_str)
            if final_num > 2 ** 31 - 1:
                return 2 ** 31 - 1
            if final_num < - 2 ** 31:
                return - 2 ** 31 
